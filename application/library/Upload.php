...
class Upload {
    ...
    public function __construct($config = array(), $driver = '') { 
        if(empty($config)){
            $config = \Yaf\Registry::get('config')->upload->toArray();
        }
        $this->config = array_merge($this->config, $config); 
        $driver = $driver ? :$this->config['type'];  
        $class = strpos($driver, '\\') ? $driver : 'Upload\\' . ucfirst(strtolower($driver));
        $this->uploader = new $class($config);
        if (!$this->uploader) {
            throw new \Yaf\Exception("不存在上传驱动：{$driver}"); 
        }
    ...
    private function getSaveName($file) {
        ...
        if (empty($rule)) { //保持文件名不变
            ...
            $savename = $this->getName($rule, $file['tmp_name']);
            if (empty($savename)) {
                $this->error = '文件命名规则错误！';
                return false;
            }
