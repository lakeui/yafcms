...
class Model {
    ...
    public function __construct($name = '', $tablePrefix = '', $connection = '') {
        ...
        $this->name=$this->name=='ZysModel'?'':$this->name;
        // 数据库初始化操作
        // 获取数据库操作对象
        // 当前模型有独立的数据库连接信息
    ...
    public function __call($method, $args) {
        if (in_array(strtolower($method), $this->methods, true)) {
            ...
            throw new Exception($method.'方法不支持');
            return;
        }
    }
    // 回调方法 初始化模型
            ...
            foreach ($data as $key => $val) {
                if (!in_array($key, $fields, true)) {
                    if (APP_DEBUG) {
                        ...
                        throw new Exception('_DATA_TYPE_INVALID_ '. ':[' . $key . '=>' . $val . ']');
                    }
                    unset($data[$key]);
            ...
            foreach ($options['where'] as $key => $val) {
                ...
                if (in_array($key, $fields, true)) {
                    ...
                    if (APP_DEBUG) {
                        ...
                        throw new Exception('_ERROR_QUERY_EXPRESS_ '. ':[' . $key . '=>' . $val . ']');
                    }
