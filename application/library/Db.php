...
class Db {
    ...
    private function parseConfig($db_config = '') {
        if (!empty($db_config) && is_string($db_config)) { 
            ...
            $config = \Yaf\Registry::get("config")->db->toArray(); 
            // 如果配置为空，读取配置文件设置
            if ($config['dsn'] && 'pdo' != strtolower($config['type'])) { // 如果设置了DB_DSN 则优先
                $db_config = $this->parseDSN($config['dsn']);
            } else {
                $db_config = array(
                    'dbms' => $config['type'],
    ...
    protected function multiConnect($master = false) {
        ...
        $config_db = \Yaf\Registry::get('config')->db->toArray(); 
        // 数据库读写是否分离
